...
public class MainActivity extends AppCompatActivity {
    ...
    public void editTask(final String oldTaskTitle) {
        final EditText taskEditText = new EditText(this);
        taskEditText.setText(oldTaskTitle);
        ...
        AlertDialog dialog = new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Update", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        String newTaskTitle = taskEditText.getText().toString();
                        if (newTaskTitle.trim().isEmpty()) {
                            Toast.makeText(MainActivity.this, "Task cannot be empty", Toast.LENGTH_SHORT).show();
                        } else {
                            updateTaskInDatabase(oldTaskTitle, newTaskTitle);
                        }
                    }
                })
                .setNegativeButton("Cancel", null)
    ...
    private void updateTaskInDatabase(String oldTitle, String newTitle) {
        SQLiteDatabase db = null;
        try {
            db = mHelper.getWritableDatabase();
            ContentValues values = new ContentValues();
            values.put(TaskContract.TaskEntry.COL_TASK_TITLE, newTitle);
            db.update(TaskContract.TaskEntry.TABLE, values, TaskContract.TaskEntry.COL_TASK_TITLE + " = ?", new String[]{oldTitle});
